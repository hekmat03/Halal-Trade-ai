...
for use_trailing in (False, True):
    label = "Donchian + Trailing Stop (3%)" if use_trailing else "Donchian (no trailing, baseline)"
    print(f"\n{'='*70}\n{label}\n{'='*70}")
    ...
    for window in windows:
        ...
        result = engine.run(make_donchian_breakout(channel_period=20), test_slice, cfg, strategy_name=label)
        r = result.report
