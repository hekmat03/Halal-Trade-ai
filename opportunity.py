...
class Opportunity:
    ...
    strategy_name: str
    confidence: float                # signal strength, 0-1 (from Signal.confidence)
    risk_reward_ratio: float         # e.g. 2.0 = risking 1 to make 2
    strategy_quality_score: float    # 0-1, from historical backtest performance
    regime_compatibility_score: float  # 0-1, how well this strategy fits current regime
    expected_fee_pct: float = 0.001  # round-trip fee+slippage estimate, as a fraction
    current_exposure_pct: float = 0.0  # 0-1, fraction of max_exposure already in use
...
class RankingWeights:
    ...
    # Penalties are subtracted, not weighted the same way as the above.
    fee_penalty: float = 1.0
    exposure_penalty: float = 0.5


def _score(opp: Opportunity, weights: RankingWeights) -> float:
    # risk_reward_ratio is unbounded above; compress it into a 0-1-ish range
    # with a soft cap so a single huge R:R claim can't dominate the score.
...
def rank_opportunities(
    ...
    """Return (opportunity, score) pairs sorted best-first. Empty in -> empty out."""
...
def select_best(
    ...
    *,
    ...
    Returns None if the list is empty or nothing clears the minimums — per
    spec section 10: "If none qualify -> HOLD." This function never forces a
    selection just because opportunities exist.
    """
    eligible = [
        opp for opp in opportunities
        if opp.confidence >= min_confidence and opp.risk_reward_ratio >= min_risk_reward
    ]
    if not eligible:
        return None
